...
const EventDetails = async ({ params, searchParams }: SearchParamProps) => {
  ...
  const [relatedEventsByCategory, relatedEventsByDepartment, relatedEventsByClub] =
    await Promise.all([
  ...
  const uniqueRelatedEvents = allRelatedEvents.filter(
    (event, index, self) =>
      index === self.findIndex((e) => e._id === event._id),
  )

  // 🔍 Detect if this image comes from UploadThing (ufs.sh / utfs.io)
  const isUploadThingImage =
    event.imageUrl?.includes('ufs.sh') ||
    event.imageUrl?.includes('utfs.io') ||
    event.imageUrl?.includes('uploadthing.com')
        ...
        <section className="flex justify-center bg-primary-50 bg-dotted-pattern">
          <div className="grid grid-cols-1 md:grid-cols-2 2xl:max-w-7xl ">
            <div className="relative w-full h-60 md:h-80 lg:h-96">
              {isUploadThingImage ? (
                // ✅ Plain <img> for UploadThing-hosted images
                <img
                  src={event.imageUrl}
                  alt={event.title}
                ...
                // ✅ next/image for all other sources
                <Image
                  src={event.imageUrl}
                  alt={event.title}
                  fill
            ...
            <div className="flex flex-col gap-2">
              ...
              <p className="p-medium-16 text-gray-600">{event.description}</p>
              Click the link below to be redirected to the Google Form of{' '}
              {event.title}
              <a
                href={event.url}
                target="_blank"
                rel="noopener noreferrer"
                className="p-medium-16 lg:p-regular-18 truncate text-blue-600 underline"
              >
                {event.url}
              </a>
            </div>
